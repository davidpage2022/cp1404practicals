...
def format_as_sentence(phrase):
    """Format a phrase as a sentence, so that it start with a capital and ends in a full stop.
    >>> format_as_sentence('hello')
    'Hello.'
    >>> format_as_sentence('It is an ex parrot.')
    'It is an ex parrot.'
    >>> format_as_sentence('to be or not to be...')
    'To be or not to be...'
    """
    return "".join((
        phrase[0].upper() if len(phrase) > 0 else "",
        phrase[1:],
        "." if phrase[-1] != "." else ""))
